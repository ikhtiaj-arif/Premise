...
import ReplyLikeEachPop from "./ReplyLikeEachPop";
const ReplyLikeUsersPop = ({ setLikePopup, userID }) => {
  return (
    <div
      data-reply-reply
      ...
      <div className="bg-[#fafafa]  rounded-[8px]  shadow-lg w-full h-[65vh] sm:h-auto sm:w-[400px] relative">
        ...
        </div>
        {
          <div className="max-h-[300px] premiseScroll overflow-y-auto  pb-6 gap-5 mt-3 mx-[30px]">
            {userID?.map((like) => (
              <ReplyLikeEachPop like={like}></ReplyLikeEachPop>
            ))}
          </div>
        }
      </div>
...
export default ReplyLikeUsersPop;
